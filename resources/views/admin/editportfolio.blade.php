    ...
    <section class="body-part01 right-bd-admin float-end">
        ...
        <div class="features-body min-height">
            <div class="headiering-steps">
                ...
                <form name="frmadd" action="{{ route('updateportfolio' , $portfolio->id) }}" method="POST" enctype="multipart/form-data">
                  @csrf
                  <input type="hidden" name="id" value="{{ $portfolio['id'] }}">
                    <div class="righty-divu spo-apce py-4">
                        <div class="d-flex align-items-center">
                            <h3 class="ms-sub"> Edit Portfolio </h3>
                        </div>
                    ...
                    <div class="form-group">
                        <label for="exampleInputPortfolioImage">User Name</label>
                        <select class="form-control" name="user_id" required>
                            <option value="">Select a User</option>
                            @foreach ($user as $user)
                                <option value="{{ $user->id }}" {{ $user->id == $portfolio->user_id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-lg-12">
                    <div class="form-group">
                        ...
                        <select id="media_type" name="media_type" class="form-control" required>
                            <option value="Video" {{ $portfolio->media_type == 'Video' ? 'selected' : '' }}>Video</option>
                            <option value="Audio" {{ $portfolio->media_type == 'Audio' ? 'selected' : '' }}>Audio</option>
                            <option value="Image" {{ $portfolio->media_type == 'Image' ? 'selected' : '' }}>Image</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <input class="form-control" type="file" id="files" name="media_files">
                        @if($portfolio->media_files)
                            <p class="mt-2">Current File: <a href="{{ asset('uploads/' . $portfolio->media_files) }}" target="_blank">View File</a></p>
                        @endif
                    </div>
                </div>
                            </div>
                            <button class="btn btn-submits btn-primary my-5"> Add Portfolio </button>
                        </div>

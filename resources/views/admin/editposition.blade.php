    ...
    <section class="body-part01 right-bd-admin float-end">
        ...
        <div class="features-body min-height">
            <div class="headiering-steps">
                ...
                <form name="frmedit" action="{{ route('updateposition', $position->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{ $position['id'] }}">
                    <div class="righty-divu spo-apce py-4">
                        <div class="d-flex align-items-center">
                            <h3 class="ms-sub"> Edit Position </h3>
                        </div>
                        @if (session('success'))
                            ...
                            <div class="row gy-4">
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        <label for="campaignName">Position Name</label>
                                        <input type="text" class="form-control" id="name" name="name" value="{{ $position->name }}" required>
                                    </div>
                                </div>
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        <label for="campaignBudget">Price</label>
                                        <input type="number" class="form-control" id="price_per_day" name="price_per_day" value="{{ $position->price_per_day }}" step="0.01" required>
                                    </div>
                                </div>
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        <label for="startDate">Dimensions</label>
                                        <input type="text" class="form-control" id="dimensions" name="dimensions" value="{{ $position->dimensions }}" required>
                                    </div>
                                </div>
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        <label for="endDate">Description</label>
                                        <input type="text" class="form-control" id="description" name="description" value="{{ $position->description }}" required>
                                    </div>
                                </div>
                            </div>
                            
                            <button class="btn btn-submits btn-primary my-5"> Update Position </button>
                        </div>
                    </div>
                </form>
